...
<body data-file="kz" <?php body_class(); echo apply_filters('polo_preloader_data',$preloader_data);?> >
<!-- WRAPPER -->
<div class="wrapper">
	...
	<section class="content no-padding-top no-padding-bottom">
		...
		polo_set_layout( $page_layout, $page_width, true );

			polo_inner_content_before();

			while ( have_posts() ) : the_post();

				// Custom Hero - slide_posts
				$heroPosts = get_post_meta($post->ID, 'slide_posts', true);
				if ($heroPosts) {
				echo do_shortcode( '[vc_row full_width="stretch_row_content_no_spaces"][vc_column]' . 
								   '[rev_slider ' . 
								   'alias="postslider-interiorpages"]' . 
								   '[gallery ids="0, ' . $heroPosts . '"]' . 
								   '[/rev_slider]' . 
								   '[/vc_column][/vc_row]');
				}
